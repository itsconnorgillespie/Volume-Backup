...
def get_filename() -> str:
    """ Build the filename using current UTC time. """
    return datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ") + ".zip"
...
def main() -> None:
    ...
    s3_key = f"{settings.S3_PREFIX}{filename}" if settings.S3_PREFIX else filename
    ...
    if not upload_s3_file(client, settings, path, s3_key):
        os.remove(path)
        sys.exit(1)
